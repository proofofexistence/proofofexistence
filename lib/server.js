...
var BLOCKCYPHER_BASEURL = 'https://api.blockcypher.com/v1/btc/' + config.networkName
var EXPECTED_CONFIRMATIONS = 1

var MailTransport = nodemailer.createTransport
var mail = new MailTransport({
  service: 'Gmail',
  auth: {
...
var DOCUMENT_PRICE_MBTC = Unit.fromSatoshis(config.DOCUMENT_PRICE).toMilis()
var DOCUMENT_PRICE_BTC = Unit.fromSatoshis(config.DOCUMENT_PRICE).toBTC()
var SIGN_PRICE_MBTC = Unit.fromSatoshis(config.SIGN_PRICE).toMilis()

var app = express()
var db = level(config.DB_PATH)
...
app.get('/developers', (req, res) => {
  res.render('developers', { title: 'API',
    ...
    documentPrice: config.DOCUMENT_PRICE
  })
})
...
app.get('/sign/:hash', (req, res) => {
  res.render('beta', { title: 'Document Information',
    active: { prove: true },
    signPriceMbtc: SIGN_PRICE_MBTC
  })
})

var BLOCKCYPHER_URL_TOKEN = '?token=' + config.BLOCKCYPHER_TOKEN
...
var makeUnconfirmedHook = address => config.HOST_URL + '/unconfirmed/' + config.MAGIC_NUMBER + '/' + address
var makeConfirmedHook = address => config.HOST_URL + '/confirmed/' + config.MAGIC_NUMBER + '/' + address
...
app.get('/api/internal/latest/confirmed', (req, res) => {
  db.get('latest-confirmed').then(results => res.send(JSON.stringify(_.map(JSON.parse(results), formatDates))))
})

app.get('/api/internal/latest/unconfirmed', (req, res) => {
  db.get('latest-unconfirmed').then(results => res.send(JSON.stringify(_.map(JSON.parse(results), formatDates))))
})
...
var doRegister = (req, res) => {
  ...
  var address = config.BASE_PRIVATE_KEY.derive(randomPath).privateKey.toAddress()
  ...
  new Promise((resolve, reject) => {
    ...
    storeObject.fee = Math.ceil(
      expectedSize * storeObject.feePerKilobyte * config.FEE_MULTIPLIER /
      1024)

    if (storeObject.fee > config.DOCUMENT_PRICE) {
      console.log('We should increase the price!', config.DOCUMENT_PRICE, 'vs', storeObject.fee)
      storeObject.fee = config.DOCUMENT_PRICE - 1
    }
...
app.post('/api/v2/appendSig/', (req, res) => {
  ...
  const append = () => {
    return db.put('sig-' + hash, JSON.stringify(sigs))
  ...
  db.get('map-' + hash).then(address => {
    ...
    if (!result.pending) {
      ...
      db.get('sig-' + hash).then(retrievedSigs => {
...
app.get('/api/v2/getSigs/:hash', (req, res) => {
  db.get('sig-' + req.params.hash).then(sigs => {
...
app.post('/confirmed/' + config.MAGIC_NUMBER + '/:address', function (req, res) {
  ...
  db.put(requestAddress + '-log-' + new Date().getTime(), JSON.stringify({
    ...
    var txNet = (config.networkName === 'test3' ? 'btc-testnet' : 'btc')
    var txUrl = `https://live.blockcypher.com/${txNet}/tx/${result.tx}`

    var mailOptions = {
      from: config.MAIL_FROM,
      to: config.MAIL_TO,
      subject: `[Proof of Existence] Document ${result.digest} verified (${config.HOST})`,
      text: `Document with hash ${result.digest} was verified in transaction ${result.tx}`,
      html: `Document with hash ${result.digest} was verified in transaction ` + `<a href="${txUrl}">${result.tx}</a> <br/>` + `Path for private key derivation is ${result.path} <br/> (${config.HOST})`
    }
...
app.get('/alldb/' + config.MAGIC_NUMBER, (req, res) => {
  db.createReadStream({})
    .on('data', data => {
...
app.post('/sweep/' + config.MAGIC_NUMBER, function (req, res) {
  var paths = ''
  var writeOps = []
...
app.post('/unconfirmed/' + config.MAGIC_NUMBER + '/:address', function (req, res) {
  var requestAddress = req.params.address
  ...
  .then(dbResult => {
    ...
    return new Promise((resolve, reject) => {
      try {
        db.get('sig-' + result.digest).then(signatures => {
          result.sighash = bitcore.crypto.Hash.sha256(Buffer.from(signatures)).toString('hex')
          resolve()
        }).catch(() => {
          resolve()
        })
  ...
  .then(requestResult => {
    ...
    if ((!result.sighash && satoshis < config.DOCUMENT_PRICE) || (result.sighash && satoshis < config.SIGN_PRICE)) {
      db.put(requestAddress + '-log-' + new Date().getTime(), JSON.stringify({
        address: requestAddress,
    ...
    if (result.sighash) {
      tx.addData(bitcore.util.buffer.concat([Buffer.from('DOCPROOF'), Buffer.from(result.sighash, 'hex')]))
    } else {
      tx.addData(bitcore.util.buffer.concat([Buffer.from('DOCPROOF'), Buffer.from(result.digest, 'hex')]))
    }
    tx.change(config.TARGET_PAYMENT_ADDRESS.derive(result.path).publicKey.toAddress())
    if (result.fee) {
      tx.fee(result.fee)
    }
    tx.sign(config.BASE_PRIVATE_KEY.derive(result.path).privateKey)
