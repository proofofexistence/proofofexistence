...
app.post('/confirmed/' + MAGIC_NUMBER + '/:address', function(req, res) {
  ...
  db.putAsync(requestAddress + '-log-' + new Date().getTime(), JSON.stringify({
    ...
    var txCur = (networkName === 'test3' ? 'tBTC' : 'BTC')
    var txUrl = `https://www.blocktrail.com/${txCur}/tx/${result.tx}`

    var mailOptions = {
      from: MAIL_FROM,
      to: MAIL_TO,
      subject: `[Proof of Existence] Document ${result.digest} verified (${HOST})`,
      text: `Document with hash ${result.digest} was verified in transaction ${result.tx}`,
      html: `Document with hash ${result.digest} was verified in transaction ` + `<a href="${txUrl}">${result.tx}</a> <br/>` + `Path for private key derivation is ${result.path} <br/> (${HOST})`
    }
