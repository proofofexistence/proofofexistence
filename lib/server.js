...
var BLOCKCYPHER_BASEURL = config.get('services.blockcypher.url') + '/btc/' + config.get('networkName')
...
var BLOCKCYPHER_URL_TOKEN = '?token=' + config.get('services.blockcypher.token')
...
var doRegister = (req, res) => {
  ...
  new Promise((resolve, reject) => {
    ...
    storeObject.fee = Math.ceil(
      expectedSize * storeObject.feePerKilobyte * config.get('feeMultiplier') /
      1024)
