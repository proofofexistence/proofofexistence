...
var SIGN_PRICE_MBTC = Unit.fromSatoshis(config.get('SIGN_PRICE')).toMilis()

var app = express()
var db = level(config.get('DB_PATH'))
...
app.post('/unconfirmed/' + config.get('MAGIC_NUMBER') + '/:address', function (req, res) {
  ...
  .then(requestResult => {
    ...
    if ((!result.sighash && satoshis < config.get('DOCUMENT_PRICE')) || (result.sighash && satoshis < config.get('SIGN_PRICE'))) {
      db.put(requestAddress + '-log-' + new Date().getTime(), JSON.stringify({
        address: requestAddress,
...
function destroy () {
  stop()
  level.destroy(config.get('DB_PATH'))
}
