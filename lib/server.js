...
var MAIL_FROM = '"Proof of Existence" <help@proofofexistence.com>'
var MAIL_USER = 'eordano.bot@gmail.com'
var MAIL_PASS = '&YM6K2XdRMoAJCU7Qu!a'
...
var app = express()
var db = bluebird.promisifyAll(new leveldb('./database'))
...
app.use('/paper-dashboard', express.static(__dirname + '/../assets/vendor/paper-dashboard/assets'))
app.use('/sprintf-js', express.static(__dirname + '/../node_modules/sprintf-js/dist'))
app.use('/openpgp', express.static(__dirname + '/../node_modules/openpgp/dist'))
app.use('/qrcodejs', express.static(__dirname + '/../node_modules/qrcodejs'))
...
var makeUnconfirmedHook = address => SERVER_URL + '/unconfirmed/' + MAGIC_NUMBER + '/' + address
var makeConfirmedHook = address => SERVER_URL + '/confirmed/' + MAGIC_NUMBER + '/' + address
...
var makeFullAddressUrl = address => BLOCKCYPHER_BASEURL + '/addrs/' + address + '/full' + BLOCKCYPHER_URL_TOKEN
...
app.post('/confirmed/' + MAGIC_NUMBER + '/:address', function(req, res) {
  ...
  db.putAsync(requestAddress + '-log-' + new Date().getTime(), JSON.stringify({
    ...
    var mailOptions = {
      ...
      html: `Document with hash ${result.digest} was verified in transaction ` + `<a href="https://insight.bitpay.com/tx/${result.tx}">${result.tx}</a> <br/>` + `Path for private key derivation is ${result.path}`
    }
