...
async function createDocproof (docproof) {
  ...
  const { balance } = await insightApi.getBalance(checkAddress)
  ...
  if (balance >= price.satoshis) {
    ...
    const addrUtxo = await insightApi.getUtxo(checkAddress)
    const payments = _.reduce(addrUtxo, (payment, tx) => {
      let outputs = {
        txId: tx.mintTxid,
    ...
    console.log(`Insufficient funds for ${paymentAddress} - ${balance}`)
    await store.putTransactionLogError(paymentAddress, balance, 'Insufficient funds')
  }
...
async function confirmDocproof (docproof) {
  const paymentAddress = bitcore.Address.fromString(docproof.payment_address)
  const checkAddress = paymentAddress.toString(true) // if BCH address, don't use prefix
  const addrFull = await insightApi.getTxs(checkAddress)

  // TODO: Check outputs for the docproof
  const spentTx = _.find(addrFull, (tx) => {
    return tx.spentTxid !== ''
  })

  if (spentTx && spentTx.spentHeight > 0) {
    const docproofTx = await insightApi.getTx(spentTx.spentTxid)
    if (docproofTx && docproofTx.confirmations > 0) {
      docproof.tx = docproofTx.txid
      docproof.blockstamp = docproofTx.blockTime
...
async function getFeePerKb () {
  const nbBlocks = '2'
  const estimatedFee = await insightApi.getEstimateFee(nbBlocks)

  const estFeePerKb = bitcore.Unit.fromBTC(estimatedFee.feerate).toSatoshis()
