...
function sendAdminDocproof (digest, txHash, derivationPath) {
  ...
  var mailOptions = {
    ...
    subject: `[${config.get('app.site.title')}-v${packageFile.version}] Document ${digest} verified (${config.get('app.url.host')})`,
...
function sendAdminPrice (oldPrice, newPrice, chainName, networkName) {
  var mailOptions = {
    from: config.get('mail.from'),
    to: config.get('mail.to'),
    subject: `[${config.get('app.site.title')}-v${packageFile.version}] Price change alert ${oldPrice} to ${newPrice}`,
