...
async function documentStatus (docproof) {
  ...
  return {
    digest: docproof.digest ? docproof.digest : '0',
    payment_address: docproof.payment_address ? docproof.payment_address : '0',
    pending: docproof.pending,
    network: config.get('app.defaultNetwork'),
    success: true,
    timestamp: docproof.timestamp ? core.util.formatDate(docproof.timestamp) : '0',
    tx: docproof.tx ? docproof.tx : '0',
    txstamp: docproof.txstamp ? core.util.formatDate(docproof.txstamp) : '0',
    blockstamp: docproof.blockstamp ? core.util.formatDate(docproof.blockstamp) : '0',
...
async function statusInBlockchain (hash) {
  ...
  if (doc.results[0] && doc.results[0].data) {
    return {
      success: true,
